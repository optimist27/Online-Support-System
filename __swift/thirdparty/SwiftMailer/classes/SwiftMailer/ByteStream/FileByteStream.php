 ...
 * @author Chris Corbyn
 */
class SwiftMailer_ByteStream_FileByteStream
  extends SwiftMailer_ByteStream_AbstractFilterableInputStream
  implements SwiftMailer_FileStream
{
  ...
  private function _getReadHandle()
  {
    if (!isset($this->_reader))
    {
      ...
      {
        throw new SwiftMailer_IoException(
  ...
  private function _copyReadStream()
  {
    ...
    {
      throw new SwiftMailer_IoException('Unable to copy the file to make it seekable, sys_temp_dir is not writable, php://memory not available');
    }
    $currentPos = ftell($this->_reader);
    fclose($this->_reader);
    ...
    if (!$source)
    {
      throw new SwiftMailer_IoException('Unable to open file for copying [' . $this->_path . ']');
    }
