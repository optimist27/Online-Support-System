 ...
 * @author Chris Corbyn
 */
abstract class SwiftMailer_Transport_AbstractSmtpTransport
  implements SwiftMailer_Transport
{
  ...
  /**
   * Set the name of the local domain which SwiftMailer will identify itself as.
   * This should be a fully-qualified domain name and should be truly the domain
   * you're using.  If your server doesn't have a domain name, use the IP in square
   * brackets (i.e. [127.0.0.1]).
   *
   * @param string $domain
   * @return SwiftMailer_Transport_AbstractSmtpTransport
   */
  ...
  /**
   * Get the name of the domain SwiftMailer will identify as.
   *
   * @return string
   */
  ...
  public function send(SwiftMailer_Mime_Message $message, &$failedRecipients = null)
  {
    ...
    if ($evt)
    {
      ...
      elseif ($sent > 0)
      {
        $evt->setResult(SwiftMailer_Events_SendEvent::RESULT_TENTATIVE);
      }
      else
      {
        $evt->setResult(SwiftMailer_Events_SendEvent::RESULT_FAILED);
      }
   ...
   *
   * @return SwiftMailer_Transport_IoBuffer
   */
  ...
  protected function _streamMessage(SwiftMailer_Mime_Message $message)
  {
    try
    {
  ...
  protected function _getReversePath(SwiftMailer_Mime_Message $message)
  {
  ...
  protected function _throwException(SwiftMailer_TransportException $e)
  {
  ...
  protected function _assertResponseCode($response, $wanted)
  {
    ...
    if (!$valid)
    {
      $this->_throwException(
        new SwiftMailer_TransportException(
  ...
  private function _sendTo(SwiftMailer_Mime_Message $message, $reversePath,
  ...
  private function _sendCc(SwiftMailer_Mime_Message $message, $reversePath,
  ...
  private function _sendBcc(SwiftMailer_Mime_Message $message, $reversePath,
