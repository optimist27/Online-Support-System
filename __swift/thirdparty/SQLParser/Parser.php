...
class SQL_Parser
{
	...
	static protected $_extendedFunctions = array(
		'customfield',
		'yesterday',
		'today',
		'tomorrow',
	...
	static protected $_extendedGroupBy = array(
		'minute',
		'hour',
		'day',
		'dayname',
	...
	static protected $_extendedGroupByFunctions = array(
		'x',
		'y',
	);
     ...
     * @uses  is_string()
     */
    public function __construct($string = null, $dialect = 'ANSI', $KQL = false)
    {
    ...
    public function getParams(&$values, &$types, $i = 0)
    {
        ...
        while ($open_braces > 0) {
			...
			/**
			 * ---------------------------------------------
			 * Added by Varun Shoor to fix lack of ) for IN statement or Functions
			 * ---------------------------------------------
			 */
			if ($_attempts > 10 && empty($this->token)) {
	...
	/**
	 * Check to see if its an extended function
	 *
	 * @author Varun Shoor
	 * @return bool "true" on Success, "false" otherwise
	 */
	public function IsExtendedFunction()
	{
		if (in_array(mb_strtolower($this->lexer->tokText), self::$_extendedFunctions)) {
			return true;
		}
    ...
    public function parseCondition()
    {
        ...
        while (true) {
            ...
            if ($this->token == '(') {
				...
				/**
				 * ---------------------------------------------
				 * Support for AS - Varun Shoor
				 * ---------------------------------------------
				 */
				if (mb_strtolower($this->lexer->tokText) == 'as') {
					$this->getTok();
					$clause['args'][count($clause['args'])-1]['alias'] = $this->lexer->tokText;
					$this->getTok();
				}

            } elseif ($this->IsExtendedFunction()) {
				...
				if (mb_strtolower($this->lexer->tokText) == 'as') {
					$this->getTok();
					$clause['alias'] = $this->lexer->tokText;
					$this->getTok();
				}
			...
			/**
			 * ---------------------------------------------
			 * Fixed by Varun Shoor for IN()
			 * ---------------------------------------------
			 */
			} else if ($this->lexer->tokText == ')') {
				$this->getTok();
            } else {
    ...
    public function parseFunctionOpts()
    {
        ...
        $this->getParamsFunction($opts['arg'], $opts['type'], $opts);
        if ($this->token != ')') {
            $this->raiseError('Expected ")"');
        }
        $this->getTok();
    ...
    // {{{ getParamsFunction(&$values, &$types, &)
    /**
     * extracts parameters from a function call
     *
     * this function should be called if an opening brace is found,
     ...
     * @uses  SQL_Parser_Lexer::$tokText R
     */
    public function getParamsFunction(&$values, &$types, &$opts, $i = 0)
    {
        $values = array();
        $types  = array();
        ...
        while ($open_braces > 0) {
			...
			$_tokenText = mb_strtolower($this->lexer->tokText);
			$_finalValue = $this->lexer->tokText;
			if (!empty($_tokenText) && isset($this->functions[$_tokenText])) {
				$this->getTok();
				if ($this->token != '(') {
					$this->raiseError('Expected "("');
				}

				$opts['args'] = array(array());
				$opts['args'][0]['name'] = $_finalValue;
				$opts['args'][0]['isfunction'] = true;
				$this->getParamsFunction($opts['args'][0]['arg'], $opts['args'][0]['type'], $opts['args'][0]);
			...
			// See if we have a field for this
			if ($this->token === 'text_val') {
				$_finalValue = "'" . $_finalValue . "'";
			}

			if ($this->token == 'text_val' && $this->KQL instanceof SWIFT_KQLParser && $this->KQL->GetIsClassLoaded()) {
				$_tableAndFieldNameContainer = $this->KQL->GetTableAndFieldNameOnText($_tokenText);
				if (isset($_tableAndFieldNameContainer[0]) && !empty($_tableAndFieldNameContainer[0]) && isset($_tableAndFieldNameContainer[1]) && !empty($_tableAndFieldNameContainer[1])) {
					$_linkedField = $this->KQL->GetLinkedToTableAndColumnInfo($_tableAndFieldNameContainer[0], $_tableAndFieldNameContainer[1], '', '');
					if (!empty($_linkedField)) {
						$_finalValue = $_linkedField;
					} else {
						$_finalValue = $_tableAndFieldNameContainer[0] . '.' . $_tableAndFieldNameContainer[1];
					}
				}
			}
			 ...
			 * ---------------------------------------------
			 */
			if ($_attempts > 20 && empty($this->token)) {
				$this->raiseError('Expected )');
			}
    ...
    public function parseIdentifier($type = 'column')
    {
        ...
        if ($prevTok != 'ident' && $prevTok != 'text_val' && $prevTok != '*') {
            $this->raiseError('Expected name (' . $this->token . ': ' . $prevTok . ')');
        }
         ...
         && $this->token != ')') {
            ...
            switch ($_baseToken) {
                ...
                case 'order':
                    ...
                    $this->getTok();
                    while ($this->token == 'ident' || $this->token == 'text_val') {
                        $arg = $this->lexer->tokText;
                        $this->getTok();
                        if ($this->token == '.') {
                            $this->getTok();
				...
				case 'multigroup':
                    ...
                    while ($this->token == 'ident' || $this->token == 'text_val' || $this->token == ':' || $this->token == '(' || $this->token == ')') {
                        $arg = $this->lexer->tokText;
						...
						if ($arg == '*end of input*') {
							...
							// Take into account the , that might come after )
							if ($arg == ',') {
								$_options = array(); // Reset the options
								$this->getTok();
								$arg = $this->lexer->tokText;
							}
						}

						$_argComparison = mb_strtolower($arg);
						if (in_array($_argComparison, self::$_extendedGroupByFunctions)) {
							$_options = array(); // Reset the options
							$_options['function'] = $arg;
							$this->getTok(); // Moves to (
							if ($this->token != '(') {
                                $this->raiseError('Expected ( got ' . $this->token);
							}
						...
						/**
						 * ---------------------------------------------
						 * Added support for extended clauses - Varun
						 * ---------------------------------------------
						 */
                        if ($this->token == ':') {
							$this->getTok();

							$_extendedGroupByToken = trim($this->lexer->tokText);

							$_isValidExtendedToken = false;
							foreach (self::$_extendedGroupBy as $_groupByToken) {
								if (substr($_groupByToken, 0, strlen($_extendedGroupByToken)) == mb_strtolower($_extendedGroupByToken)) {
									$_isValidExtendedToken = true;
								}
							}

							if (!$_isValidExtendedToken) {
								$this->raiseError('Unexpected extended group by clause');
							}

							$_options['extended'] = mb_strtolower(trim($_extendedGroupByToken));

							$this->getTok();

						} else if ($this->token == ')' && isset($_options['function'])) {
							$this->getTok();
						}
                        $tree[$_baseToken . '_by'][] = array($col, $_options);
