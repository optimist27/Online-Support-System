...
if (!class_exists('FUNC5'))
	require_once 'FUNC5.php';

class MIME5 {
	const LE = PHP_EOL;	// Line end sequence
	const FOLD = "\r\n\t";	// Header folding sequence
	const HLEN = 78;	// Header line length
	const MLEN = 70;	// Message body line length
	...
	static public function is_printable($str = null, $debug = null) {
		...
		if (!is_string($str))
			FUNC5::trace($debug, 'invalid argument type');
		else {
			...
			if ($str == '')
				...
				foreach (explode($end, $str) as $line) {
					if ($line == '')
						...
						for ($i = 0; $i < $mcnt; $i++)
						{
							// Handle space specially if it appears at the end of a line
							if (' ' == substr($match[0][$i], -1))
								$line = substr($match[0][$i], 0, -1) . '=20';
							else
								$line = $match[0][$i];
	...
	static public function encode_header($str = null, $charset = null, $encoding = null, $len = null, $end = null, $debug = null) {
		...
		else if (!(strlen($charset) >= 2 && FUNC5::is_alpha($charset, true, '-')))
			...
			if ($str == '')
				...
				if ($encoding == 'quoted-printable') {
					if (!self::is_printable($str)) {
						...
						$enc = self::qp_encode($str, $new, self::FOLD);
						$enc = str_replace(array('?', ' ', '=' . self::FOLD), array('=3F', '_', self::FOLD), $enc);
					}
					...
					$cnk = rtrim(chunk_split($str, $new, self::FOLD));
					$imp = array();
					foreach (explode(self::FOLD, $cnk) as $line)
						if ($line != '')
							$imp[] = base64_encode($line);
					$enc = implode(self::FOLD, $imp);
				}
				$res = array();
				if ($enc) {
					$chr = ($encoding == 'base64') ? 'B' : 'Q';
					foreach (explode(self::FOLD, $enc) as $val)
						if ($val != '')
							$res[] = '=?' . $charset . '?' . $chr . '?' . $val . '?=';
				} else {
					$cnk = rtrim(chunk_split($str, $len, self::FOLD));
					foreach (explode(self::FOLD, $cnk) as $val)
						if ($val != '')
							$res[] = $val;
				}
				return implode(self::FOLD, $res);
			}
		}
	}
				...
				foreach (preg_split('/(?<!\\?(?i)q)\\?\\=/', $str, -1, PREG_SPLIT_NO_EMPTY) as $str1) {
					foreach (explode('=?', $str1, 2) as $str2) {
						...
						if (count($exp = explode('?B?', $str2)) == 2) {
							if (strlen($exp[0]) >= 2 && FUNC5::is_alpha($exp[0], true, '-') && trim($exp[1]) != '')
							...
							if (strlen($exp[0]) >= 2 && FUNC5::is_alpha($exp[0], true, '-') && $exp[1] != '')
	...
	static public function compose($text = null, $html = null, $attach = null, $uniq = null, $end = null, $debug = null) {
		...
		if ($text == null && $html == null)
			...
			if ($text != null) {
				if (!(is_array($text) && isset($text['header'], $text['content']) && is_string($text['header']) && is_string($text['content']) && self::isset_header($text['header'], 'content-type', 'text/plain', $debug)))
					$err[] = 'invalid text message type';
			}
			if ($html != null) {
				if (!(is_array($html) && isset($html['header'], $html['content']) && is_string($html['header']) && is_string($html['content']) && self::isset_header($html['header'], 'content-type', 'text/html', $debug)))
			...
			if ($multipart) {
				...
				if ($attach != null) {
					foreach ($attach as $darr) {
						if (self::isset_header($darr['header'], 'content-disposition', 'inline', $debug))
							$disp['inline'] = true;
						else if (self::isset_header($darr['header'], 'content-disposition', 'attachment', $debug))
							$disp['attachment'] = true;
					}
				}
				$hstr = 'Content-Type: multipart/%s;' . $end . "\t" . 'boundary="%s"';
				$bstr = '--%s' . $end . '%s' . $end . $end . '%s';
				$body[] = 'This is a message in MIME Format.' . $end . 'If you see this, your mail reader does not support this format.' . $end;
				if ($text && $html) {
					if ($disp['inline'] && $disp['attachment']) {
						...
						foreach ($attach as $desc)
							if (self::isset_header($desc['header'], 'content-disposition', 'inline', $debug))
								$body[] = sprintf($bstr, $boundary2, $desc['header'], $desc['content']);
						$body[] = '--' . $boundary2 . '--';
						foreach ($attach as $desc)
							if (self::isset_header($desc['header'], 'content-disposition', 'attachment', $debug))
	...
	static public function isset_header($str = null, $name = null, $value = null, $debug = null) {
		...
		if (!(is_string($name) && strlen($name) > 1 && FUNC5::is_alpha($name, true, '-')))
			$err[] = 'invalid name type';
	...
	static public function split_message($str = null, $debug = null) {
		if (!FUNC5::is_debug($debug))
			$debug = debug_backtrace();
		if (!(is_string($str) && $str != ''))
			FUNC5::trace($debug, 'invalid message value');
		else {
			$ret = false;
			if (strpos($str, "\r\n\r\n"))
				$ret = explode("\r\n\r\n", $str, 2);
	...
	static public function split_mail($str = null, &$headers, &$body, $debug = null) {
		if (!FUNC5::is_debug($debug))
			$debug = debug_backtrace();
		$headers = $body = false;
		if (!$part = self::split_message($str, $debug))
			return false;
		...
		if (substr($type, 0, strlen('multipart/')) == 'multipart/' && $boundary && strstr($part['content'], '--' . $boundary . '--'))
			$body = self::_parts($part['content'], $boundary, strtolower(substr($type, strlen('multipart/'))), $debug);
		if (count($body) == 0)
			$body[] = self::_content($str, $debug);
	}
	...
	static private function _parts($str = null, $boundary = null, $multipart = null, $debug = null) {
		...
		$err = array();
		if (!(is_string($str) && $str != ''))
			$err[] = 'invalid content value';
		if (!(is_string($boundary) && $boundary != ''))
			$err[] = 'invalid boundary value';
		if (!(is_string($multipart) && $multipart != ''))
			$err[] = 'invalid multipart value';
		if (count($err) > 0)
			FUNC5::trace($debug, implode(', ', $err));
		else {
			$ret = array();
						...
						if ($cnt > 1 && $part = self::split_message($split, $debug)) {
							if ($harr = self::split_header($part['header'], $debug)) {
								...
								if (substr($type, 0, strlen('multipart/')) == 'multipart/' && $newb && strstr($part['content'], '--' . $newb . '--'))
									$ret = self::_parts($part['content'], $newb, $multipart . '|' . strtolower(substr($type, strlen('multipart/'))), $debug);
								else {
	...
	static private function _content($str = null, $debug = null) {
		...
		if (!(is_string($str) && $str != ''))
			...
			if ($encoding == 'base64' || $encoding == 'quoted-printable')
				$body['content'] = self::decode_content($part['content'], $encoding, $debug);
			else {
				if ($encoding)
