...
class MAIL5 {
	...
	public function send($resc = null, $debug = null) {
		...
		if (count($err) > 0) FUNC5::trace($debug, implode(', ', $err));
			...
			if (count($arr) > 0) {
				$to = implode(', ', $arr);
				$header['client'][] = 'To: '.implode(', '.MIME5::FOLD, $arr);
			...
			if (count($this->Cc) > 0) {
				...
				if (count($arr) > 0) {
					...
					$header['client'][] = 'Cc: '.implode(', '.MIME5::FOLD, $arr);
				}
			}
			$hbcc = '';
			if (count($this->Bcc) > 0) {
			...
			if (is_int($this->Priority) || is_string($this->Priority)) {
				...
				if ($arr) {
					...
					$header['local'][] = 'X-MimeOLE: Produced By Kayako '. SWIFT_PRODUCT . ' v'. SWIFT_VERSION; // << required by SpamAssassin in conjunction with "X-MSMail-Priority"
					$header['local'][] = 'X-Mailer: Kayako '. SWIFT_PRODUCT . ' v'. SWIFT_VERSION;
					$header['client'][] = 'X-Priority: '.$arr[0];
					$header['client'][] = 'X-MSMail-Priority: '.$arr[1];
					$header['client'][] = 'X-MimeOLE: Produced By Kayako '. SWIFT_PRODUCT . ' v'. SWIFT_VERSION;
					$header['client'][] = 'X-Mailer: Kayako '. SWIFT_PRODUCT . ' v'. SWIFT_VERSION;
				}
			}
			$header['client'][] = 'Message-ID: <'.MIME5::unique().'@'. SWIFT_Mail::GetMailLocalhost() .'>';
			if (count($this->Header) > 0) {
			...
			if ($delivery == 'relay') {
				...
				if (ini_get('safe_mode')) {
				    $res = mail($to, $subject, $arr['content'], implode(MIME5::LE, $header['local']).MIME5::LE.$arr['header']);
				}else {
