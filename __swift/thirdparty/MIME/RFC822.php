...
class Mail_RFC822
{
    ...
    */
    ...
    protected function _splitAddresses($address)
    {
		...
		// Patch for bug reported by client [Varun] varun.shoor@kayako.com
		if (is_array($address)) {
			$_returnEmailAddress = false;
			
			foreach ($address as $_emailAddress) {
				if (empty($_returnEmailAddress)) {
					$_returnEmailAddress = $_emailAddress;
				}
				
				$this->addresses[] = array('address' => trim($_emailAddress), 'group' => false);
			}
    ...
    protected function _validateQuotedString($qstring)
    {
		...
		// Perform check.
		// These characters must be escaped in a quoted-string:
		// \
		// "
		// -- RML
	    
		// Walk the string backwards checking for un-escaped chars.
		for ($n = strlen($qstring) - 1; $n >= 0; $n--)
		{
			if ($qstring[$n] == '\\' || $qstring[$n] == '"')
			{
				if ($n == 0 || $qstring[$n - 1] != '\\')
				{
					// Unescaped.
				...
				{
					// Preceding character is a slash; skip it as it is
					// the escape sequence for this char.
					$n--;
				}
			}
		}
